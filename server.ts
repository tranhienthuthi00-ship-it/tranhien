...
CRITICAL: Return ONLY a raw JSON object matching the requested schema. No markdown formatting, no explanations, no wrappers.`;
  ...
  app.post("/api/charlie-style/convert", async (req, res) => {
    ...
    try {
      console.log("[Charlie Style] Received convert request");
      let imagePart: any = null;
      const matches = imageBase64.match(/^data:([a-zA-Z0-9]+\/[a-zA-Z0-9-.+]+);base64,(.+)$/);
      if (matches && matches.length === 3) {
        imagePart = {
      ...
      // Step 1: Use gemini-3.5-flash to analyze the picture and get standard visual description
      console.log("[Charlie Style] Analyzing image composition using gemini-3.5-flash...");
      const descResult = await getGeminiClient().models.generateContent({
        model: "gemini-3.5-flash",
        contents: [
          {
            role: "user",
            parts: [
              imagePart,
              { text: "Analyze this image and describe its subject composition, focal elements, characters or objects, and their actions. Write a compact, detailed English description (max 80 words) optimized to recreate the scene with an image generator." }
            ]
          }
        ]
      });

      const description = descResult.text?.trim() || "A cute character sitting in a happy mood.";
      console.log("[Charlie Style] Analysis description:", description);

      // Step 2: Formulate the Quentin Blake prompt
      const quentinPrompt = `A whimsical, playful drawing in the signature style of Quentin Blake (illustrator of Roald Dahl's books like Matilda, Charlie and the Chocolate Factory). It depicts: ${description}. Hand-drawn with scratchy black dip-pen ink lines, loose and spontaneous splashy watercolor wash paint inside and overflowing slightly, light airy composition, quirky asymmetrical character design, expressive facial features, spindly thin limbs. Set against a clean, completely plain solid white background. Delightful children's book illustration.`;
      console.log("[Charlie Style] Prompt formulated:", quentinPrompt);

      // Step 3: Call gemini-2.5-flash-image to generate the image
      console.log("[Charlie Style] Generating image using gemini-2.5-flash-image...");
      
      let generatedImageBase64 = "";
      try {
        const imageResult = await getGeminiClient().models.generateContent({
        ...
        console.warn("[Charlie Style] Image generation API issue/paid-quota limitation or permissions issue:", genErr.message);
        // We will pass the description and custom prompt to the frontend, letting it fall back gracefully
      }

      res.json({
        success: true,
        description,
        prompt: quentinPrompt,
        imageUrl: generatedImageBase64
      });

    } catch (error: any) {
      console.error("[Charlie Style] Error converting:", error);
      res.status(500).json({ error: error.message || "Đã xảy ra lỗi khi phân tích ảnh." });
    }
  });
