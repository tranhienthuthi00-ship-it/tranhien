...
export function DigitalJournal({ 
  ...
  const [showTodoSortMenu, setShowTodoSortMenu] = useState(false);
            ...
            <div className="lg:col-span-2 flex flex-col pt-2 relative z-10 w-full mb-10">
             ...
             <div className="lg:col-span-1 flex flex-col pt-16 xl:pl-4">
                 ...
                 <div className="space-y-4">
                   ...
                   {/* Sorting Filter Dropdown at the top of the Todo list */}
                   <div className="relative inline-block text-left mb-3">
                     <button
                       type="button"
                       onClick={() => setShowTodoSortMenu(!showTodoSortMenu)}
                       className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-[#8A1E2B]/20 bg-[#FAF3EB]/65 hover:bg-[#8A1E2B]/5 font-hand font-bold text-xs text-[#8A1E2B] transition-all duration-200 cursor-pointer shadow-sm"
                       title="Sắp xếp danh sách"
                     >
                       <Filter className="w-3.5 h-3.5" />
                       <span className="text-[#8A1E2B]/75">Sắp xếp:</span>
                       <span className="text-[#8A1E2B] font-black uppercase">
                         {todoSortOrder === 'priority' ? '🎯 Ưu tiên' : todoSortOrder === 'difficulty' ? '🔥 Độ khó' : 'Mặc định'}
                       </span>
                     </button>
                     ...
                     {showTodoSortMenu && (
                       <>
                         ...
                         <div 
                           ...
                           onClick={() => setShowTodoSortMenu(false)}
                         />
                         
                         <div className="absolute left-0 mt-1 w-44 rounded-xl shadow-lg bg-[#FAF3EB] border-2 border-[#8A1E2B]/25 py-1.5 z-50 animate-fade-in font-hand font-bold text-xs text-[#8A1E2B]">
                           <button
                             type="button"
                             onClick={() => {
                               setTodoSortOrder('default');
                               setShowTodoSortMenu(false);
                             }}
                             className={`w-full text-left px-4 py-2 hover:bg-[#8A1E2B]/5 transition-colors flex items-center justify-between cursor-pointer ${todoSortOrder === 'default' ? 'bg-[#8A1E2B]/10 text-[#8A1E2B]' : ''}`}
                           >
                             <span>MẶC ĐỊNH</span>
                             {todoSortOrder === 'default' && <Check className="w-3.5 h-3.5 stroke-[3px]" />}
                           </button>
                           
                           <button
                             type="button"
                             onClick={() => {
                               setTodoSortOrder('priority');
                               setShowTodoSortMenu(false);
                             }}
                             className={`w-full text-left px-4 py-2 hover:bg-[#8A1E2B]/5 transition-colors flex items-center justify-between cursor-pointer ${todoSortOrder === 'priority' ? 'bg-[#8A1E2B]/10 text-[#8A1E2B]' : ''}`}
                           >
                             <span>🎯 ƯU TIÊN</span>
                             {todoSortOrder === 'priority' && <Check className="w-3.5 h-3.5 stroke-[3px]" />}
                           </button>
                           
                           <button
                             type="button"
                             onClick={() => {
                               setTodoSortOrder('difficulty');
                               setShowTodoSortMenu(false);
                             }}
                             className={`w-full text-left px-4 py-2 hover:bg-[#8A1E2B]/5 transition-colors flex items-center justify-between cursor-pointer ${todoSortOrder === 'difficulty' ? 'bg-[#8A1E2B]/10 text-[#8A1E2B]' : ''}`}
                           >
                             <span>🔥 ĐỘ KHÓ</span>
                             {todoSortOrder === 'difficulty' && <Check className="w-3.5 h-3.5 stroke-[3px]" />}
                           </button>
                         </div>
                       </>
                     )}
